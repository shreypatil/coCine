...
# --- prepare the box, BEFORE installing anything -----------------------------
# Order matters and was originally wrong here as it was in setup.sh: every
# measure below makes the install cheaper, and all of them used to run after it.
#
# Swap first. dnf reached a 748 MB resident set on the signalling box and was
# OOM-killed against the 498 MB swapfile the image ships; coturn's install is
# smaller but the metadata parse is identical, so the same ceiling applies.
say () { printf '\n\033[1m==> %s\033[0m\n' "$*"; }
say "swap"
...
#
# coturn and certbot are EPEL packages -- neither is in Oracle Linux's default
# repositories, and a plain `dnf install coturn certbot` fails outright with
# "Unable to find a match". Oracle ships the repo definition in
# oracle-epel-release-el9, which these images already carry, but leaves
# `ol9_developer_EPEL` disabled. Enabled for this transaction only rather than
# permanently: it is a developer repository and should not quietly feed later
# updates on a box that is meant to be predictable.
PKG="nice -n 19 ionice -c3"
if command -v dnf >/dev/null; then
  $PKG dnf --disablerepo=ol9_ksplice --enablerepo=ol9_developer_EPEL \
      install -y coturn firewalld certbot
else
